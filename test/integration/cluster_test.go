...
var _ = Describe("Cluster Lifecycle", Serial, func() {
	Describe("Single-Node Cluster Creation", func() {
		...
		It("should create and initialize a complete Kubernetes cluster", func() {
			...
			By("Verifying port 6443 is published")
			portPublished := false
			for _, port := range container.Ports {
				if strings.Contains(port, "6443") {
					portPublished = true
					break
				}
			}
			Expect(portPublished).To(BeTrue(), "Port 6443 should be published")
	...
	Describe("Cluster Stop Isolation", Serial, func() {
		var clusterName string
