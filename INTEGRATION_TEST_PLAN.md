...
- **Full suite** (with merged tests): ~8-12 minutes (down from ~15-20 min)
- **Parallel execution**: ~5-8 minutes (down from ~10-12 min)

**Time Savings from Test Merging:** ~40-50% reduction in total test execution time

### Parallel vs Serial

- **Parallel** (default): Tests with unique cluster names run concurrently
- **Serial** (marked): Resource-intensive tests (multi-node, HA clusters)
